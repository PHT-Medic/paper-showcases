...
def training(cfgs_name=None, save_dir=None, gpus=None, prev_station=None):
    ...
    os.getcwd()
    if cfgs_name is None:
        pc_cfg = importlib.import_module('pc_cfgs.'+sys.argv[1])
    else:
        pc_cfg = importlib.import_module('pc_cfgs.' + cfgs_name)
    ...
    if save_dir is None:
        ...
        # TODO add to relative on train
        mdlParams['saveDirBase'] = mdlParams['saveDir'] + sys.argv[2]

    else:
        model_cfg = importlib.import_module('cfgs.' +save_dir)
        mdlParams_model = model_cfg.init(mdlParams)
        mdlParams.update(mdlParams_model)
        mdlParams['saveDirBase'] = mdlParams['saveDir'] + save_dir
    ...
    if gpus is None:
        ...
        if 'gpu' in gpus:
            # mdlParams['numGPUs'] = [[int(s) for s in re.findall(r'\d+', gpus)][-1]]
            mdlParams['numGPUs'] = [int(s) for s in re.findall(r'\d+', gpus)]
            print(f'train.py: numGPUS = {mdlParams["numGPUs"]}')
            cuda_str = ""
            for i in range(len(mdlParams['numGPUs'])):
                cuda_str = cuda_str + str(mdlParams['numGPUs'][i])
                if i is not len(mdlParams['numGPUs'])-1:
    ...
    for cv in cv_set:
        ...
        if mdlParams['balance_classes'] < 3 or mdlParams['balance_classes'] == 7 or mdlParams['balance_classes'] == 11:
            ...
            if mdlParams['numClasses'] == 9:  # TODO changed to 8 from 9 (UKN Class)
        ...
        # Load trained model
        # meta_features_model TODO: Currently leaving out for simplicity (Flo)
        ...
        print('Multi GPU?')
        # multi gpu support
        if len(mdlParams['numGPUs']) > 1 and torch.cuda.device_count() > 1:
            print(f'train.py: Using {len(mdlParams["numGPUs"])} GPUs. -> nn.DataParallel!')
        ...
        if load_old:
            ...
            if prev_station:  # TODO: Load pretrained model from given path
                print(f'Loading pretrained "{prev_station}" model from previous station!')
                state = torch.load(prev_station)
                modelVars['model'].load_state_dict(state['state_dict'])
                print('Loaded model from previous station!')
            start_epoch = 1
            mdlParams['lastBestInd'] = -1
            # Track metrics for saving best model
            mdlParams['valBest'] = 1000

        # TODO: Evaluate the labels
        print(np.sum(mdlParams['labels_array'], axis=0))
        ...
        for step in range(start_epoch, mdlParams['training_steps']+1):
            ...
            if step % mdlParams['display_step'] == 0 or step == 1:
                ...
                if mdlParams['classification']:
                    ...
                    # Save in mat
                    save_dict['loss'].append(loss.item())
                    ...
                    # Additionally save the train loss!
                    save_dict['train_loss'].append(np.mean(epoch_loss))
                ...
                if mdlParams['classification']:
                    ...
                    print("Loss on ",eval_set,"set: ",loss.item()," Accuracy: ",accuracy," F1: ",f1," (best WACC: ",-mdlParams['valBest']," at Epoch ",mdlParams['lastBestInd'],")")
                    ...
                    if mdlParams['print_trainerr'] and 'train' not in eval_set:
                        ...
                        # Save in mat
                        save_dict_train['loss'].append(loss.item())
        ...
        # Train on the validation Set at the end!
        modelVars['model'].train()
        for j, (inputs, labels, indices) in enumerate(modelVars['dataloader_valInd']):
            # print(indices)
            # t_load = time.time()
            # Run optimization
            if mdlParams.get('meta_features', None) is not None:
        ...
        # Save model after training on validation set
        ...
        torch.save(state, mdlParams['saveDir'] + '/checkpoint-final+val.pt')

        # Free everything in modelvars
        modelVars.clear()
        # After CV Training: print CV results and save them

        cv_performance = {"BestF1": allData['f1Best'][cv],
                          "BestSens": allData['sensBest'][cv],
                          "BestAcc": allData['accBest'][cv]}
        performance[str(cv)] = cv_performance
    ...
    # Values of first station (stopped exec) TODO change values for central train
    # Get local results if already computed
    local_res = glob(mdlParams['saveDirBase'] + '/results_**.pkl')
    try:
        local_res_file = Path(local_res[0])
        if local_res_file.exists():
            with open(local_res[0], 'rb') as res_file:
                performance = pickle.load(file=res_file)
            print("Use local precomputed results")
