...
def init(mdlParams_):
    print('Using effb6_fk_st2_equaldist.py')
    mdlParams = {}
    ...
    ### Model Selection ###
    mdlParams['model_type'] = 'efficientnet-b6'
    mdlParams['dataset_names'] = ['official']#,'sevenpoint_rez3_ll']
    mdlParams['file_ending'] = '.jpg'  # TODO Important!!!
    ...
    mdlParams['numCV'] = 5  # TODO to take only 1 train and test set
    mdlParams['trans_norm_first'] = True
    # Scale up for b1-b7
    # mdlParams['input_size'] = [224,224,3]  # original_size
    mdlParams['input_size'] = [380,380,3]

    ### Training Parameters ###
    # Batch size
    mdlParams['batchSize'] = 25#*len(mdlParams['numGPUs'])
    ...
    # All sets (based on new indeces -> nothing to change
    allSets = glob(path1 + '*/')   
    # Go through all sets
    for i in range(len(allSets)):
        ...
        # with open('/mnt/vdb/data/isic/2019/labels/official/labels_fk.csv', newline='') as csvfile:
            # print(f'training.py: files[j] = {files[j]}')
            print(f'Using file "{csvfile}" as labels.')
            labels_str = csv.reader(csvfile, delimiter=',', quotechar='|', lineterminator='\r\n')
    ...
    for i in range(len(allSets)):
        ...
        # TODO: Self rewritten!
        mdlParams['labels_list'] = list(mdlParams['labels_dict'].values())
        mdlParams['key_list'] = list(mdlParams['labels_dict'].keys())
        for i, k in enumerate(mdlParams['key_list']):
            found = False
            for file in files:
                if '.jpg' in file or '.jpeg' in file or '.JPG' in file or '.JPEG' in file or '.png' in file or '.PNG' in file:
                    f = k + mdlParams['file_ending']
                    if f in file:
                        found = True
                        mdlParams['im_paths'].append(file)
                        # Aktuell unnecessary
                        if mdlParams['exclude_inds']:
                            for key in indices_exclude:
                                if key in file:
                                    exclude_list.append(indices_exclude[key])
            if found == False:
                print(f'Image {k} could not be found!')
    ...
    ### Define Indices ###
    # ind_file = 'indices_isic2019_og.pkl'  # Original Indices
    # ind_file = 'all_indices.pkl'
    ind_file = 's_2indices.pkl'  # Indices station specific
    with open(mdlParams['saveDir'] + ind_file,'rb') as f:  # TODO updated indices file - train needs to define
        print(f'Loading file {ind_file} as indices file!')
        indices = pickle.load(f)
    mdlParams['trainIndCV'] = indices['trainIndCV']
    mdlParams['valIndCV'] = indices['valIndCV']
    print("Class distribution labels file: {}".format(np.sum(mdlParams['labels_array'], axis=0)))
    print("Len labels array: {}".format(len(mdlParams['labels_array'])))
    ...
    for i in range(len(mdlParams['trainIndCV'])):
        ...
        print(
            "Train class distribution fold {}: {}".format(0, np.sum(mdlParams['labels_array'][mdlParams['trainIndCV'][i]], axis=0)))
    print("Val")
    for i in range(len(mdlParams['valIndCV'])):
        print(mdlParams['valIndCV'][i].shape)
        print("Val class distribution fold {}: {}".format(i, np.sum(mdlParams['labels_array'][mdlParams['valIndCV'][i]], axis=0)))
        print("Sum indices val fold {}: {}".format(i, np.sum(mdlParams['valIndCV'][i])))
